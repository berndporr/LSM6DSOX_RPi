...
void LSM6DSOX::start ()
{
    ...
    gpiod_line_settings_set_direction (
        settings, GPIOD_LINE_DIRECTION_INPUT); //input line
    gpiod_line_settings_set_edge_detection (
        settings, GPIOD_LINE_EDGE_RISING); //detect rising edge
    line_cfg = gpiod_line_config_new ();
    if (gpiod_line_config_add_line_settings (line_cfg, &LMS6DSOX_DRDY_GPIO, 1,
                                             settings)
        < 0)
    { //offset is the gpio pin (22)
...
void LSM6DSOX::worker ()
{
    ...
    while (running)
    {
        int r = gpiod_line_request_wait_edge_events (request,
                                                     -1); //wait indefinitely
        if (DEBUG)
        {
...
void LSM6DSOX::contiguousReadBytes (uint8_t address, uint8_t *container,
                                    uint8_t nBytes)
{
...
uint8_t LSM6DSOX::i2cReadByte (uint8_t address)
{
    ...
    ssize_t w = write (
        fd, buffer,
        1); //test write (have to do it or it can't read, don't know why)
...
void LSM6DSOX::initAccelerometer ()
{
    ...
    bits = (xlSettings.samplingRate & 0x0F)
           << 4; //shift 4 bits back and write to ODR[3:0]
    if (!bits)
    {
    ...
    bits |= xlScaleBits ();
    i2cWriteByte (LSM6DSOX_CTRL1_XL,
                  bits); //only writing sampling rate for now
    xlRes = getXlRes ();
}
...
const char *LSM6DSOX::throwStatus (uint8_t statusByte)
{
    ...
    {
    case (0x01):
        return "Accelerometer bit set high!";
        break;
    case (0x02):
        return "Gyro bit set high!";
        break;
    case (0x03):
        return "Accelerometer and gyro bit set high!";
        break;
    case (0x04):
        return "Temp bit set high!";
        break;
    case (0x05):
        return "Temp and accelerometer bit set high!";
        break;
    case (0x06):
        return "Temp, gyro and accelerometer bit set high!";
        break;
    default:
...
void LSM6DSOX::registerCallback (LSM6DSOXCallback cb) { callback = cb; }
...
uint8_t LSM6DSOX::gyroScaleBits ()
{
    ...
    { //assumes FS_125 bit in CTRL2_G low
    case GyroSettings::GYRO_250_DPS:
        gyro_bits = 0x00;
        break;
    case GyroSettings::GYRO_500_DPS:
        gyro_bits = 0x01;
        break;
    case GyroSettings::GYRO_1000_DPS:
        gyro_bits = 0x02;
        break;
    case GyroSettings::GYRO_2000_DPS:
        gyro_bits = 0x03;
        break;
    default:
        throw "Invalid scale settings";
        break;
    }
...
uint8_t LSM6DSOX::xlScaleBits ()
{
    ...
    case XLSettings::XL_2G:
        ...
        break;
    case XLSettings::XL_4G:
        xl_bits = 0x02;
        break;
    case XLSettings::XL_8G:
        xl_bits = 0x03;
        break;
    case XLSettings::XL_16G:
        xl_bits = 0x01;
        if (fs)
        {
            std::cout << "Warning: Setting scale to 2g because FS bit is high"
                      << std::endl;
        }
        break;
    default:
        throw "Invalid XL scale setting";
        break;
    }
    return xl_bits << 2;
}
